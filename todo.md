...
## UI 全面优化（第二轮）
- [x] 全局样式：优化色彩变量、阴影层次、圆角体系、字体引入
- [x] 登录页：重构为左右分栏布局，左侧品牌区+右侧表单
- [x] 侧边栏：优化品牌 Logo、导航项样式、用户信息区
- [x] 仪表盘：优化统计卡片、图表区域、快速操作区
- [x] 设备管理页：优化设备卡片、状态徽章、空状态
- [x] 任务管理页：优化任务列表、新建任务弹窗、进度展示
- [x] 任务详情页：优化结果表格、过滤器、统计图表
- [x] IOC管理页：优化上传区域、规则集列表卡片
